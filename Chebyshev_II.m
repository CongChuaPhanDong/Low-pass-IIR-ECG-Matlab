%% ================================================================
%  THIẾT KẾ BỘ LỌC SỐ IIR THÔNG THẤP - CHEBYSHEV LOẠI II
%  Low-pass IIR Digital Filter Design using Chebyshev Type II
%% ================================================================
clc; clear; close all;
...
dlgtitle = 'Thông số bộ lọc Chebyshev II IIR';
...
%  Chebyshev II dùng Rs (suy hao stopband) để xác định bậc
%% ---------------------------------------------------------------
...
%  cheby2: gợn ở STOPBAND (khác Chebyshev I gợn ở passband)
%% ---------------------------------------------------------------
[b, a] = cheby2(N, Rs, Wn, 'low');

fprintf('\n===== HỆ SỐ BỘ LỌC =====\n');
fprintf('Tử số  b = ['); fprintf(' %.6f', b); fprintf(' ]\n');
fprintf('Mẫu số a = ['); fprintf(' %.6f', a); fprintf(' ]\n');
...
if all(abs(p) < 1)
    fprintf('=> Bộ lọc ỔN ĐỊNH (tất cả cực nằm trong vòng đơn vị)\n');
else
...
%% ---------------------------------------------------------------
figure('Name','Bộ lọc IIR Chebyshev II Thông Thấp', ...
...
sgtitle(sprintf('Bộ lọc IIR Thông Thấp Chebyshev II  |  N = %d  |  fs = %.0f Hz  |  fp = %.0f Hz', ...
    N, fs, fp), 'FontSize', 13, 'FontWeight','bold');

%% ---------------------------------------------------------------
%  7. MÔ PHỎNG LỌC TÍN HIỆU (Signal Filtering Demo)
%% ---------------------------------------------------------------
f1 = max(1, round(fp * 0.5));
f2 = min(round(fs_stop * 1.5), floor(fs/2) - 10);
t  = 0 : 1/fs : 1 - 1/fs;
...
sgtitle(sprintf('Mô phỏng lọc tín hiệu – Chebyshev II Low-pass IIR  |  fp = %.0f Hz', fp), ...
